...
An interesting observation here is the MDS of only the data related to pressure. We can see that the effect of the other flood factors is largely negated and pressure dominated the distances in MDS. It would be interesting to realize the effects of pressure later in the next analysis. 
...
We can see that adding pressure alters the principal components too. Lets also present the results of clustering. For clustering, we first try *MClust* method in R to identify the algorithm that performs the best clustering and see if both the datasets return the same clusters.
